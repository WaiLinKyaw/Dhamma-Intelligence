...
def get_db_connection():
    if not DATABASE_URL:
        # Fallback for local testing (Local တွင် postgres မရှိသေးပါက)
        raise ValueError("DATABASE_URL environment variable မတွေ့ရှိပါ။")
...
def init_db():
    ...
    """)
    # ပထမအကြိမ် Database တည်ဆောက်ချိန်တွင် Default Quotes အချို့ ထည့်သွင်းပေးခြင်း
...
def main():
    ...
    if job_queue:
        ...
        job_queue.run_daily(noon_job, time=datetime.time(hour=12, minute=00, tzinfo=mm_tz))
        job_queue.run_daily(noon_job, time=datetime.time(hour=13, minute=50, tzinfo=mm_tz))
        job_queue.run_daily(noon_job, time=datetime.time(hour=14, minute=00, tzinfo=mm_tz))
