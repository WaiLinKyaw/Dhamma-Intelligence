...
# ----------------- DATABASE HELPERS -----------------
# Database URL ကို Environment Variable မှ ဆွဲယူခြင်း
# (Render ပေါ်တွင် DATABASE_URL ရှိမည်၊ Local စက်တွင်မရှိပါက local sqlite သို့မဟုတ် hardcoded url သုံးနိုင်သည်)
DATABASE_URL = os.environ.get("DATABASE_URL")
# Render ရဲ့ "postgres://" ကို psycopg2 နားလည်တဲ့ "postgresql://" အဖြစ် အလိုအလျောက် ပြောင်းလဲပေးခြင်း
...
def get_db_connection():
    if not DATABASE_URL:
        raise ValueError("DATABASE_URL environment variable မရှိသေးပါခင်ဗျာ။ Render Environment တွင် ထည့်သွင်းပေးပါ။")
    return psycopg2.connect(DATABASE_URL)

def init_db():
    """PostgreSQL Table များ မရှိသေးပါက အလိုအလျောက် တည်ဆောက်ပေးသည့် Function"""
    ...
    # 2. daily_quotes table (SERIAL သို့ ပြောင်းထားပါသည်)
    cursor.execute("""
    ...
    # 3. dhamma_resources table (SERIAL သို့ ပြောင်းထားပါသည်)
    cursor.execute("""
    ...
    # Default Quotes အချို့ ထည့်သွင်းပေးခြင်း
    cursor.execute("SELECT COUNT(*) FROM daily_quotes")
    if cursor.fetchone()[0] == 0:
        default_quotes = [
            ("လက်ရှိ ပစ္စုပ္ပန်တည့်တည့်မှာ သတိဖြင့် အေးချမ်းပါစေ။",),
            ("စိတ်၏ ဖြစ်ပျက်ကို သတိဖြင့် စောင့်ကြည့်ပါ။ အားလုံးသည် အနိစ္စသာ ဖြစ်ပါသည်။",),
            ("ဒေါသဖြစ်လာလျှင် ချက်ချင်း တုံ့ပြန်ခြင်းမပြုဘဲ ဝင်လေ ထွက်လေကို သတိကပ်ပါ။",)
        ]
        cursor.executemany("INSERT INTO daily_quotes (quote) VALUES (%s)", default_quotes)
...
def register_user(chat_id: int, first_name: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    # PostgreSQL တွင် INSERT OR IGNORE အစား ON CONFLICT DO NOTHING သုံးပါသည်
    cursor.execute("""
        INSERT INTO bot_users (chat_id, first_name, subscribed)
        VALUES (%s, %s, 1)
...
def get_random_daily_quote():
    conn = get_db_connection()
    cursor = conn.cursor()
    # PostgreSQL တွင် RANDOM() ကို သုံးပါသည်
...
def get_resources(category: str, r_type: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    # SQLite ရဲ့ '?' နေရာတွင် PostgreSQL အတွက် '%s' ကို အသုံးပြုပါသည်
    cursor.execute(
        "SELECT title, source FROM dhamma_resources WHERE category = %s AND type = %s", 
        (category, r_type)
    )
...
def get_all_resources(r_type: str):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT title, source FROM dhamma_resources WHERE type = %s", (r_type,))
    results = cursor.fetchall()
    cursor.close()
    conn.close()
    return results
...
async def broadcast_quote(context: ContextTypes.DEFAULT_TYPE, title: str):
    ...
    # Render ပေါ်မှာ DB အလွတ်ဖြစ်နေရင်တောင် သင့်ဆီရောက်အောင် စမ်းသပ်နည်း
    # (သင့် Chat ID ကို ဤနေရာတွင် အစားထိုးပါ)
    MY_CHAT_ID = 1619184108  # <--- သင့် Telegram User ID အမှန်ထည့်ပါ
    if MY_CHAT_ID not in users:
        users.append(MY_CHAT_ID)
    logging.info(f"Broadcast စတင်ပို့နေပါသည်... စုစုပေါင်း Users: {len(users)}")
    if not users:
        logging.warning("သတိပေးချက်: စာပို့ရန် bot_users table ထဲတွင် မည်သူမျှ မရှိသေးပါ။ (/start အရင်နှိပ်ပါ)")
        return
...
def main():
    ...
    if job_queue:
        ...
        job_queue.run_daily(morning_job, time=datetime.time(hour=7, minute=00, tzinfo=mm_tz))
        job_queue.run_daily(noon_job, time=datetime.time(hour=12, minute=00, tzinfo=mm_tz))
        job_queue.run_daily(noon_job, time=datetime.time(hour=16, minute=00, tzinfo=mm_tz))
        job_queue.run_daily(evening_job, time=datetime.time(hour=19, minute=00, tzinfo=mm_tz))
